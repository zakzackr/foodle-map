...
1. **認証機能**
    ...
    - ログアウト
2. **投稿機能**
    - 投稿（Instagram ライクな UI で写真と文章を投稿
    - 投稿の編集
    - 投稿の削除
    - 記事の下書き保存
3. **Feed 機能**
    - フォローしている人の投稿を表示（一枚以上の画像と文章を投稿。写真が複数枚のときは、横にスクロールして二枚目以降を表示。文章が一定の長さを超えた時、「続きを読む」などをクリックしたときに全文を表示）
    - おすすめの投稿を表示
4. **レビュー 機能**

-   投稿に飲食店のレビューを星 0-5 で設定できる
-   投稿に設定した飲食店のレビューを編集できる
-   投稿に設定した飲食店のレビューを削除できる
...
8. **フォロー機能**
    ...
    - プロフィールからユーザーをフォローできる
    - プロフィールからユーザーのフォローを解除できる

9. **Map 機能**
    - 飲食店詳細表示（評価・レビューなどを表示）
10. **プロフィール 機能**
    - プロフィール表示（アイコン、bio、Location）
    - プロフィール編集
